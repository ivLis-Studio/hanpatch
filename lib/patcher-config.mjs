...
const ENGINE_RULES = new Map([
  ["psp-iso-delta-v2", { format: "PSPDELTA", version: 2, mode: "file" }],
  ["p2kofsp1-v2", { format: "P2KOFSP1", version: 2, mode: "file" }],
  ["psp-file-set-delta-v1", { format: "P2KODLC1", version: 1, mode: "directory" }],
]);
export const SUPPORTED_ENGINES = new Set(ENGINE_RULES.keys());
...
function validateIsoEngineData(value, rule) {
  ...
  if (data.format !== rule.format || data.formatVersion !== rule.version) {
    throw new Error(`엔진 ${rule.format} 형식 버전 ${rule.version}이 필요합니다.`);
  }
...
function validateFileSetEngineData(value, rule) {
  const data = requireObject(value, "engineData");
  requireString(data.format, "engineData.format", 32);
  if (data.format !== rule.format || data.formatVersion !== rule.version) {
    throw new Error(`엔진 ${rule.format} 형식 버전 ${rule.version}이 필요합니다.`);
  ...
  for (const [index, entryValue] of data.entries.entries()) {
    const entry = requireObject(entryValue, `engineData.entries[${index}]`);
    const name = requireString(entry.name, `engineData.entries[${index}].name`, 255);
    if (basename(name) !== name || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,254}$/.test(name)) {
      throw new Error(`파일 세트 이름이 올바르지 않습니다: ${name}`);
    }
    if (names.has(name)) throw new Error(`파일 세트 이름이 중복됩니다: ${name}`);
    names.add(name);
    for (const key of ["sourceSize", "targetSize"]) {
      if (!Number.isSafeInteger(entry[key]) || entry[key] <= 0 || entry[key] > 128 * 1024 * 1024) {
        throw new Error(`engineData.entries[${index}].${key} 값이 올바르지 않습니다.`);
      }
    }
    requireSha256(entry.sourceSha256, `engineData.entries[${index}].sourceSha256`);
    requireSha256(entry.targetSha256, `engineData.entries[${index}].targetSha256`);
  }
  if (data.requiresMainPatch !== undefined) requireString(data.requiresMainPatch, "engineData.requiresMainPatch", 32);
}
...
export function validatePatcherConfig(value, expectedSlug = null) {
  ...
  const engineRule = ENGINE_RULES.get(config.engine);

  const input = requireObject(config.input, "input");
  const inputMode = input.mode ?? "file";
  if (inputMode !== engineRule.mode) throw new Error(`엔진 ${config.engine}의 입력 모드는 ${engineRule.mode}이어야 합니다.`);
  ...
  if (engineRule.mode === "directory") validateFileSetEngineData(config.engineData, engineRule);
  else validateIsoEngineData(config.engineData, engineRule);
