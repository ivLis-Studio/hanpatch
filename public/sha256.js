  ...
  class Sha256 {
    ...
    update(input) {
      ...
      if (this.bufferLength > 0) {
        const amount = Math.min(64 - this.bufferLength, data.length);
        this.buffer.set(data.subarray(0, amount), this.bufferLength);
      ...
      while (position + 64 <= data.length) {
        this.#compress(data.subarray(position, position + 64));
        position += 64;
      }
    ...
    #compress(block) {
      ...
      for (let index = 16; index < 64; index += 1) {
        const left = w[index - 15];
        const right = w[index - 2];
      ...
      for (let index = 0; index < 64; index += 1) {
        ...
        const first = (h + bigSigma1 + choice + K[index] + w[index]) >>> 0;
